    ...
    $scope.getLocations = function () {
        $http.get("/api/Location/getLocations").then(function (response, req) {
            ...
            if ($scope.Location != null) {
                $scope.loc = $scope.Location[0];
                $scope.GetLocationImages($scope.Location[0].id);
                if ($scope.parentImgs != null) {
                    $scope.getImagePreview($scope.parentImgs[0].Id);
                }
            }

        });
    }
    $scope.GetLocationquestion = function () {
        $http.get("/api/question/GetLocationquestion?locationId=" + $scope.loc.id).then(function (response, req) {
            $scope.Locationquestion = response.data;
            $scope.Ans = null;
        });
    }
